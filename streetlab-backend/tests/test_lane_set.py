...
def test_the_containment_predicate_holds_at_its_tolerance_boundary():
    """The tolerance pinned by a test, not only by the constant's comment.
    ...
    exactly, flush against the kerb edge; the same change stops fitting once the
    ego is more than `LANE_FIT_TOL_M` further right than that. Measured worst
    case on either scene is 0.66 m of slack required -- `Route.offset`'s mitre
    scaling at a corner -- so the boundary sits 0.09 m beyond what the fixtures
    actually ask for.
    """
    assert lane_change_is_legal(-1.8, 2, 2, -1)
    assert not lane_change_is_legal(-1.8, 2, 2, +1)
    assert lane_change_is_legal(-1.8 - LANE_FIT_TOL_M, 2, 2, -1)
    assert not lane_change_is_legal(-1.8 - LANE_FIT_TOL_M - 1e-9, 2, 2, -1)
